...
**In-person mode.** For a face-to-face meeting on one iPad, the rep picks **In person** on the consent screen instead of sending a link — no video call, no second device. Deepgram streaming diarizes the room, then each run's **own audio** is scored against the rep's voice (session-adapted from the enrolled on-device voiceprint) and the learned customer voice, so a customer line no longer waits for a fixed 3 s window; text cues and the always-wins toggle and per-line ⇄ correction remain as fallbacks. Rep-first is only the default when there is no working voiceprint, and `?voicedebug=1` shows the rep an in-memory table of each run's decision. The voice engine runs entirely on the iPad (NeXt-TDNN on onnxruntime-web): its 30 s audio ring lives only in worker memory and is discarded continuously; no audio is stored, and the only new stored values are two similarity scalars (`selfMean` and `otherMean`) on the rep's own profile. The customer reads the same disclosures and consents on the shared iPad. **Sharing mode** then lets the rep turn the iPad round: tapping *Show customer* (or a citation under a suggested line) swaps the prompter for a customer-facing whiteboard — an explainer card for the concept in play, the actual brochure page it is grounded in rendered from the PDF with the cited figures highlighted, and pen/highlighter tools to draw over it. Only customer-safe text is ever shown (a concept's plain-language statement, the clause text, the page), drawings are never persisted, and recording keeps running until *Back to my view*. Everything downstream — ledger, alerts, readiness, Understanding Record — is identical; only the capture and display layers change. Design notes: `docs/in-person-mode-research.md`.
...
| `/rep/voice` | rep (protected) | One-time voice enrolment for in-person attribution: record a ~40 s sample; only the voiceprint is stored, on-device matching, no audio kept; optional second-voice calibration stores one similarity number (no audio). |
