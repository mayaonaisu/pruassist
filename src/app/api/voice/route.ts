...
import { saveVoiceProfile, loadVoiceProfile, deleteVoiceProfile } from "@/lib/voice-profile";
...
export async function GET() {
  ...
  return NextResponse.json({ profile: rec?.profile ?? null, model: rec?.model ?? null, updatedAt: rec?.updatedAt ?? null });
}
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const rec = await saveVoiceProfile(rep.sub as string, profile, model);
